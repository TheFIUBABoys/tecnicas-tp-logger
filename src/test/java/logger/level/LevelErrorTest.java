...
public class LevelErrorTest {
    ...
    public void testCompareToInfo() throws Exception {
    	assertTrue(LogLevel.LEVEL_INFO.compareTo(LogLevel.LEVEL_ERROR) > 0);
    }

    @Test
    public void testCompareToWarn() throws Exception {
    	assertTrue(LogLevel.LEVEL_WARN.compareTo(LogLevel.LEVEL_ERROR) > 0);
    }

    @Test
    public void testCompareToError() throws Exception {
        assertTrue(LogLevel.LEVEL_ERROR.compareTo(LogLevel.LEVEL_ERROR) < 0);
    }

    @Test
    public void testCompareToFatal() throws Exception {
        assertTrue(LogLevel.LEVEL_FATAL.compareTo(LogLevel.LEVEL_ERROR) < 0);
    }

    @Test
    public void testCompareToOff() throws Exception {
        assertTrue(LogLevel.LEVEL_OFF.compareTo(LogLevel.LEVEL_ERROR) > 0);
    }
